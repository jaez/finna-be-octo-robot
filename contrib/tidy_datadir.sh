...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Dgdollartest2 database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Dgdollartest2 datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Dgdollartest2 datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Dgdollartest2 0.7 datadir."
    ;;
  3)
    echo "Detected Dgdollartest2 pre-0.8 datadir."
    ;;
  4)
    echo "Detected Dgdollartest2 0.8 datadir."
    ;;
esac
