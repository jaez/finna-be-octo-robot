...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = Dgdollartest2Units::DGD;
    ...
    l2->setText(Dgdollartest2Units::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(Dgdollartest2Units::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(Dgdollartest2Units::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));        // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText((fLowOutput ? (fDust ? tr("DUST") : tr("yes")) : tr("no"))); // Low Output / Dust
    l8->setText(Dgdollartest2Units::formatWithUnit(nDisplayUnit, nChange));        // Change
    ...
    l8->setStyleSheet((nChange > 0 && nChange < CENT) ? "color:red;" : ""); // Change < 0.01DGD
        
    // tool tips
    l5->setToolTip(tr("This label turns red, if the transaction size is bigger than 5000 bytes.\n\n This means a fee of at least %1 per kb is required.\n\n Can vary +/- 1 Byte per input.").arg(Dgdollartest2Units::formatWithUnit(nDisplayUnit, CTransaction::nMinTxFee)));
    l6->setToolTip(tr("Transactions with higher priority get more likely into a block.\n\nThis label turns red, if the priority is smaller than \"medium\".\n\n This means a fee of at least %1 per kb is required.").arg(Dgdollartest2Units::formatWithUnit(nDisplayUnit, CTransaction::nMinTxFee)));
    l7->setToolTip(tr("This label turns red, if any recipient receives an amount smaller than %1.\n\n This means a fee of at least %2 is required. \n\n Amounts below 0.546 times the minimum relay fee are shown as DUST.").arg(Dgdollartest2Units::formatWithUnit(nDisplayUnit, CENT)).arg(Dgdollartest2Units::formatWithUnit(nDisplayUnit, CTransaction::nMinTxFee)));
    l8->setToolTip(tr("This label turns red, if the change is smaller than %1.\n\n This means a fee of at least %2 is required.").arg(Dgdollartest2Units::formatWithUnit(nDisplayUnit, CENT)).arg(Dgdollartest2Units::formatWithUnit(nDisplayUnit, CTransaction::nMinTxFee)));
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            {
                sAddress = CDgdollartest2Address(outputAddress).ToString().c_str();
                
                // if listMode or change => show dgdollartest2 address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, Dgdollartest2Units::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, Dgdollartest2Units::format(nDisplayUnit, nSum));
