...
{
    QString ret = QString("dgdollartest2:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non DGD unit input in lnReqAmount, we generate the URI with DGD as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(Dgdollartest2Units::format(Dgdollartest2Units::DGD, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
