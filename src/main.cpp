...
uint256 hashGenesisBlock("0x006efe01232f9f90c4f563780917ca0c9f4f32f603cc5fa9f692828fa1dd300d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dgdollartest2: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dgdollartest2 Signed Message:\n";
...
{
    // Dgdollartest2: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 600000 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 700800); // Dgdollartest2: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 86400; // Dgdollartest2: 3.5 days
static const int64 nTargetSpacing = 45; // Dgdollartest2: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dgdollartest2: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("dgdollartest2-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dgdollartest2: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Dgdollartest2: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x006efe01232f9f90c4f563780917ca0c9f4f32f603cc5fa9f692828fa1dd300d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "0db571b8b280f41bacd6ed5534daf8c0";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1404806056;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 864047440;
        ...
        assert(block.hashMerkleRoot == uint256("0xae05aa2a4c963805bdc700e0b9e0193a922296272e0afa864640db9353c15bf5"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Dgdollartest2: increase each by adding 2 to dgdollartest2's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("Dgdollartest2Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("Dgdollartest2Miner : ProcessBlock, block not accepted");
    }

    return true;
}

void static Dgdollartest2Miner(CWallet *pwallet)
{
    printf("Dgdollartest2Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dgdollartest2-miner");
    ...
    try { loop {
        ...
        printf("Running Dgdollartest2Miner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("Dgdollartest2Miner terminated\n");
        throw;
    }
}

void GenerateDgdollartest2s(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&Dgdollartest2Miner, pwallet));
}
